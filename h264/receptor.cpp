...
void *envia( void *ptr)
{
	...
	decoder  = gst_element_factory_make ("rtph264pay", "decoder"); 
	sink     = gst_element_factory_make ("udpsink", "video-output"); 
	
	g_object_set (sink, "port", RTP_TX, "host", DEST_HOST,NULL);
//	g_object_set(decoder, "pt" , 96, "config-interval", 10 , NULL);
	...
	g_object_set (G_OBJECT (source), "location", "ref.mp4", NULL); 
...
void *recibe( void *ptr)
{
  ...
  mp4mux = gst_element_factory_make  ("avdec_h264", "mp4mux");
  filesink = gst_element_factory_make  ("autovideosink", "filesink");
